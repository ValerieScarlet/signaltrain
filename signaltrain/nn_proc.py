...
class signalNet(nn.Module):
    def __init__(self, num_channels, dilation_depth, num_repeat, num_knobs , kernel_size=2):
        ...
        self.embed = CausalConv1d(
            ...
            kernel_size=num_knobs,
        )
        self.cond_hidden = _conv_stack(dilations, num_channels, internal_channels, 1)
        #self.cond_residuals = _conv_stack(dilations, num_channels, num_channels, 1)
    ...
    def forward(self, x, knobs):
        ...
        h = knobs.unsqueeze(1)
        h = self.embed(h)
        #h = knobs.unsqueeze(1).repeat(1,self.num_channels,1)
        

        #for hidden, residual, cond_hidden, cond_residual in zip(self.hidden, self.residuals, self.cond_hidden, self.cond_residuals):
        for hidden, residual, cond_hidden in zip(self.hidden, self.residuals, self.cond_hidden):
            x = out
            out_hidden = hidden(x)
            #y = h
            h_hidden = cond_hidden(h)

            # gated activation
            #   split (32,16,3) into two (16,16,3) for tanh and sigm calculations
            out_hidden_split = torch.split(out_hidden, self.num_channels, dim=1)
            h_hidden_split = torch.split(h_hidden.repeat(1,1,int(out_hidden.size()[2])), self.num_channels, dim=1)
            out = torch.tanh(out_hidden_split[0] + h_hidden_split[0]) * torch.sigmoid(out_hidden_split[1] + h_hidden_split[1])
            skips.append(out)

            out = residual(out)
            out = out + x[:, :, -out.size(2) :]

            #h_split = torch.split(h_hidden, self.num_channels, dim=1)
            #h = torch.tanh(h_split[0]) * torch.sigmoid(h_split[1])
            #h = cond_residual(h) + y
